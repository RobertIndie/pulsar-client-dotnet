...
type ControlledClusterFailover
    ...
    let run (ctx: IServiceInfoProviderContext) =
        backgroundTask {
            use httpClient = new HttpClient(HttpClientHandler(AllowAutoRedirect = true))
            ...
            while not cts.IsCancellationRequested do
                try
                    ...
                    if response.IsSuccessStatusCode then
                        ...
                        let! failoverResponse = JsonSerializer.DeserializeAsync<ControlledFailoverResponse>(responseStream, jsonOptions)
