    ...
    do (backgroundTask {
        ...
        while continueLoop do
            ...
            | Stop ->
                ...
                transactionClient |> Option.iter (fun client -> client.Close())
    ...
    member private this.MultiTopicSubscribeAsync (consumerConfig: ConsumerConfiguration<'T>, schema: ISchema<'T>, interceptors: ConsumerInterceptors<'T>) =
        backgroundTask {
            ...
            let! partitionsForTopics =
                ...
                |> Seq.map (fun topic -> this.GetConsumerInitInfo(schema, topic))
                |> Task.WhenAll
            let! consumer = MultiTopicsConsumerImpl.InitMultiTopic(consumerConfig, currentConfig, connectionPool, partitionsForTopics,
