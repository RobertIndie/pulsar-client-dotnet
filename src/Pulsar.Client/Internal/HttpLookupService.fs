...
type private HttpStatusException(statusCode: HttpStatusCode) =
    inherit Exception(sprintf "HTTP request failed with status code %O" statusCode)
...
type internal PulsarHttpClient () =
    ...
    let httpClient = new HttpClient(HttpClientHandler(AllowAutoRedirect = true))

    member this.Get<'T> (requestUri: string, auth: Authentication) : Task<'T> =
        backgroundTask {
            ...
            if authenticationDataProvider.HasDataForHttp() then
                ...
                if not response.IsSuccessStatusCode then
                    raise (HttpStatusException response.StatusCode)
                use! responseStream = response.Content.ReadAsStreamAsync()
                return! JsonSerializer.DeserializeAsync<'T>(responseStream, jsonOptions)
            else
    ...
    member private this.GetSchemaInner(topicName: CompleteTopicName, schemaVersion: SchemaVersion option,
        ...
        async {
            ...
            with Flatten ex ->
                ...
                | :? HttpStatusException as ex when ex.StatusCode = HttpStatusCode.NotFound ->
